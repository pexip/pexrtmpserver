...
rtmp_server_notify_connection (GObject * server, Client * client)
{
  ...
  if (client->publisher) {
    g_signal_emit (srv, pex_rtmp_server_signals[SIGNAL_ON_PUBLISH],
        0, client->id, client->path, client->params, &reject);
  } else {
...
static void
rtmp_server_add_client_by_id (PexRtmpServer * srv, Client * client)
{
  g_mutex_lock (&srv->client_by_id_lock);
...
static void
rtmp_server_remove_client_by_id (PexRtmpServer * srv, Client * client)
{
  g_mutex_lock (&srv->client_by_id_lock);
  g_hash_table_remove (srv->client_by_id, GINT_TO_POINTER (client->id));
  g_mutex_unlock (&srv->client_by_id_lock);
}
    ...
    PexRtmpClientID client_id)
{
  gboolean ret = FALSE;
  g_mutex_lock (&srv->client_by_id_lock);
  Client *client = g_hash_table_lookup (srv->client_by_id,
      GINT_TO_POINTER (client_id));
  if (client) {
...
{
  Client *client = client_new (G_OBJECT (srv), srv->client_id, srv->connections,
      srv->stream_id, srv->chunk_size, rtmp_server_notify_connection);
  rtmp_server_add_client_by_id (srv, client);
  return client;
}
...
pex_rtmp_server_dialout (PexRtmpServer * srv,
    ...
    gint src_port)
{
  return pex_rtmp_server_dialout_with_timeout (srv, src_path, url, addresses,
      src_port, 0);
}
...
{
  return pex_rtmp_server_external_connect_with_timeout (srv, src_path, url,
      addresses, FALSE, src_port, connect_timeout_ms);
}
...
pex_rtmp_server_dialin (PexRtmpServer * srv,
    ...
    gint src_port)
{
  return pex_rtmp_server_dialin_with_timeout (srv, src_path, url, addresses,
      src_port, 0);
}
...
{
  return pex_rtmp_server_external_connect_with_timeout (srv, src_path, url,
      addresses, TRUE, src_port, connect_timeout_ms);
}

gboolean
pex_rtmp_server_add_direct_publisher (PexRtmpServer * srv, const gchar * path)
{
...
gboolean
pex_rtmp_server_add_direct_subscriber (PexRtmpServer * srv, const gchar * path)
{
  g_mutex_lock (&srv->direct_lock);
    ...
    Client * client, PexRtmpServerStatus reason)
{
  ...
  if (client->retry_connection) {
    ...
    client->connect_timeout_ts = (client->connect_timeout_val_ms) ?
        ...
        (client->connect_timeout_val_ms * (G_USEC_PER_SEC / 1000)) : 0;
    gst_atomic_queue_push (srv->pending_clients, client);
    return;
  }

  gchar *path = g_strdup (client->path);
  gchar *params = g_strdup (client->params);
  ...
  if (srv->running && !not_notified) {
    if (publisher) {
      g_signal_emit (srv, pex_rtmp_server_signals[SIGNAL_ON_PUBLISH_DONE],
          0, client_id, path, params, reason);
    } else {
      g_signal_emit (srv, pex_rtmp_server_signals[SIGNAL_ON_PLAY_DONE],
          0, client_id, path, params, reason);
    }
  }
...
{
  return pex_rtmp_server_external_connect_with_timeout (srv, src_path, url,
      addresses, is_publisher, src_port, 0);
}
    ...
    const gboolean is_publisher, gint src_port, guint32 connect_timeout_ms)
{
  ...
  client->connect_timeout_val_ms =
      (connect_timeout_ms ==
      0) ? DEFAULT_TCP_SYN_TIMEOUT_MS : connect_timeout_ms;
  client->connect_timeout_ts =
      g_get_monotonic_time () +
      (client->connect_timeout_val_ms * (G_USEC_PER_SEC / 1000));
  GST_DEBUG ("Using %s connect timeout of %ums, timeout_timestamp:%"
      G_GUINT64_FORMAT, (connect_timeout_ms == 0) ? "default" : "configured",
      client->connect_timeout_val_ms, client->connect_timeout_ts);
...
rtmp_server_do_poll (PexRtmpServer * srv)
{
  ...
  if (srv->port > INVALID_PORT
      && gst_poll_fd_can_read (srv->fd_set, &srv->listen_gfd)) {
    rtmp_server_create_client (srv, srv->listen_gfd.fd);
    return TRUE;
  }
  if (srv->ssl_port > INVALID_PORT
      && gst_poll_fd_can_read (srv->fd_set, &srv->listen_ssl_gfd)) {
    ...
    if (client->state == CLIENT_TCP_HANDSHAKE_IN_PROGRESS
      ...
      if (g_get_monotonic_time () >= client->connect_timeout_ts) {
        GST_WARNING_OBJECT (srv,
            ...
            client->connect_timeout_val_ms, client, client->path,
            client->publisher);
        rtmp_server_remove_client (srv, client,
...
pex_rtmp_server_init (PexRtmpServer * srv)
{
  ...
  g_mutex_init (&srv->client_by_id_lock);
  ...
  srv->direct_publishers =
      g_hash_table_new_full (g_str_hash, g_str_equal, g_free,
      (GDestroyNotify) _client_disconnect);
  srv->direct_subscribers =
      g_hash_table_new_full (g_str_hash, g_str_equal, g_free,
      (GDestroyNotify) _client_disconnect);
  srv->client_by_id = g_hash_table_new (NULL, NULL);
...
pex_rtmp_server_finalize (GObject * obj)
{
  ...
  g_hash_table_destroy (srv->client_by_id);
  ...
  g_mutex_clear (&srv->client_by_id_lock);
...
pex_rtmp_server_class_init (PexRtmpServerClass * klass)
{
  ...
  pex_rtmp_server_signals[SIGNAL_ON_PLAY] =
      ...
      G_TYPE_BOOLEAN, 3, G_TYPE_INT, G_TYPE_STRING, G_TYPE_STRING);

  pex_rtmp_server_signals[SIGNAL_ON_PLAY_DONE] =
      g_signal_new ("on-play-done", PEX_TYPE_RTMP_SERVER,
      G_SIGNAL_RUN_LAST, 0, NULL, NULL, g_cclosure_marshal_generic,
      G_TYPE_NONE, 4, G_TYPE_INT, G_TYPE_STRING, G_TYPE_STRING, G_TYPE_UINT);
...
gint
pex_rtmp_server_get_port (const PexRtmpServer * srv)
{
...
gint
pex_rtmp_server_get_ssl_port (const PexRtmpServer * srv)
{
  gint port = INVALID_PORT;
