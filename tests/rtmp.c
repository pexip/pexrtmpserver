...
GST_START_TEST (rtmp_speex_flv_end_to_end)
{
  ...
  h->sink_harness =
      gst_harness_new_parse ("speexdec ! pexaudioconvert ! pexcisionaudiosink");

  /* configure the sink */
  GValueArray *freq_list = NULL;
  GstElement *sink =
      gst_harness_find_element (h->sink_harness, "pexcisionaudiosink");
  g_signal_connect (sink, "freq-list", G_CALLBACK (array_from_element_cb),
      &freq_list);
  g_object_set (sink, "fft-mag-threshold", -25.0, "fft-required-samples", 960,
      NULL);
...
GST_START_TEST (rtmp_flv_timestamping_basics)
{
  ...
  g_signal_connect (h->element, "pad-added", G_CALLBACK (flvdemux_pad_added),
      h);
...
GST_START_TEST (rtmp_window_size)
{
  ...
  Client *client =
      client_new (0, connections, (GObject *) server, FALSE, 1337, 128, NULL);
...
GST_START_TEST (rtmp_server_dialin)
{
  ...
  rtmp_harness_dialin (h_bob, bob, h_abe, abe, "rtmp", "localhost",
      "127.0.0.1");
...
GST_START_TEST (rtmp_server_dialin_and_dialout)
{
  ...
  rtmp_harness_dialout (h_bob, bob_subscriber, h_abe, abe_publisher, "rtmp",
      "localhost", "127.0.0.1");
  rtmp_harness_wait_for_notified_subscribers (h_abe, 1);
  rtmp_harness_dialin (h_bob, bob_publisher, h_abe, abe_subscriber, "rtmp",
      "localhost", "127.0.0.1");
